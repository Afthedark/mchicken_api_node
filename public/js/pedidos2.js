document.addEventListener('alpine:init', () => {
    Alpine.data('gestorPedidosGeneral', () => ({
        todosLosPedidos: [],
        paginaActual: 1,
        pedidosPorPagina: 24,
        ...
        async cargarPedidos(isFullLoad = false) {
            if (isFullLoad && this.todosLosPedidos.length === 0) {
                 // loading state handled purely via transition if we wanted, but let's keep it simple
            }
            try {
                ...
                let parsedPedidos = rawPedidos.filter(p => !completados.includes(this.getPedidoIdUnico(p)))
        ...
        limpiarLocalStoragePedidos() {
            ...
            // the UI notification requires element styling which varies, using a console log for backup
